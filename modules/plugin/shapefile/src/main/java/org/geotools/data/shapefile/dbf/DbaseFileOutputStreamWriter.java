 ...
 */
public class DbaseFileOutputStreamWriter implements Closeable {
    private DbaseFileHeaderOutputStream header;
     ...
     */
    public DbaseFileOutputStreamWriter(DbaseFileHeaderOutputStream header, WritableByteChannel out)
            throws IOException {
     ...
     * @throws IOException If errors occur while initializing.
     */
    public DbaseFileOutputStreamWriter(
            DbaseFileHeaderOutputStream header, WritableByteChannel out, Charset charset)
            throws IOException {
     ...
     */
    public DbaseFileOutputStreamWriter(
            DbaseFileHeaderOutputStream header,
    ...
    public DbaseFileOutputStreamWriter(
        ...
        header.writeHeader(this.outputStream);
        this.header = header;
        // this.channel = out;
    ...
    private void write() throws IOException {
        ...
        if (!isOutputStream) {
            ...
            while (r > 0) {
                this.outputStream.write(buffer.get());
                r = buffer.remaining();
            }
        }
    }
    ...
    public DbaseFileHeaderOutputStream getHeader() {
        return this.header;
    }
